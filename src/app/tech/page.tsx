...
export default function TechPage() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      {/* Hero Section */}
      <section className="py-12 md:py-16 lg:py-24 bg-gradient-to-br from-background via-secondary/20 to-background">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-4xl mx-auto">
            <div className="flex justify-center mb-4 md:mb-6">
              <Badge className="bg-secondary text-primary px-3 md:px-4 py-1 md:py-2 text-sm">
                Technical Specifications
              </Badge>
            </div>
            <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-light text-foreground mb-4 md:mb-6">
              Our Tech
            </h1>
      ...
      <section className="py-12 md:py-16 lg:py-20">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-8 md:mb-12">
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-light text-foreground mb-3 md:mb-4">
              Technical Architecture
            </h2>
            ...
            {Object.entries(technicalSpecs).map(([key, spec]) => (
              <button
                ...
                className={`px-4 md:px-6 py-2 md:py-3 rounded-lg text-xs sm:text-sm font-medium transition-all duration-300 ${
                  activeSpec === key ? "bg-primary text-primary-foreground shadow-lg" : "bg-secondary text-muted-foreground hover:bg-accent"
                }`}>
                {spec.title}
              </button>
            ))}
          ...
          <div className="max-w-6xl mx-auto">
            <Card className="border-border bg-card shadow-lg">
              <CardContent className="p-4 md:p-6 lg:p-8">
                <h3 className="text-xl md:text-2xl font-medium text-foreground mb-6 md:mb-8">
                  ...
                  {technicalSpecs[activeSpec].content.map((item, index) => (
                    <div key={index} className="border-l-4 border-primary pl-4 md:pl-6">
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-3 md:mb-4 gap-2">
                        <h4 className="text-base md:text-lg font-medium text-foreground">{item.component}</h4>
                        <Badge variant="outline" className="text-primary border-border w-fit">
                          {item.version}
                        </Badge>
                      </div>
                      <p className="text-muted-foreground mb-3 md:mb-4 text-sm md:text-base">{item.details}</p>
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2 md:gap-3">
                        {item.specs.map((spec, specIndex) => (
                          <div key={specIndex} className="flex items-center text-xs md:text-sm text-muted-foreground">
                            <CheckCircle className="w-3 md:w-4 h-3 md:h-4 text-primary mr-2 flex-shrink-0" />
      ...
      <section className="py-12 md:py-16 lg:py-20 bg-background">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-8 md:mb-12">
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-light text-foreground mb-3 md:mb-4">Integration APIs</h2>
            <p className="text-muted-foreground max-w-2xl mx-auto px-4">
          ...
          <div className="max-w-7xl mx-auto">
            <div className="grid lg:grid-cols-3 gap-6 md:gap-8">
              {integrationAPIs.map((category, index) => (
                <Card key={index} className="border-border bg-card shadow-lg h-full">
                  <CardContent className="p-6 md:p-8">
                    <div className="flex items-center mb-6">
                      <Network className="w-6 h-6 mr-3 text-primary" />
                      <h3 className="text-xl font-medium text-foreground">{category.category}</h3>
                    </div>
                    <div className="space-y-4">
                      {category.apis.map((api, apiIndex) => (
                        <div key={apiIndex} className="p-4 bg-secondary rounded-lg border border-border">
                          <div className="flex items-center justify-between mb-2">
                            <h4 className="font-medium text-foreground text-base">{api.name}</h4>
                            <Badge className="bg-primary/20 text-primary text-xs">{api.status}</Badge>
                          </div>
                ...
                <CardContent className="p-6 md:p-8">
                  <div className="grid lg:grid-cols-5 gap-6 md:gap-8">
                    <div className="lg:col-span-2">
                      <h3 className="text-xl font-medium text-foreground mb-4">{option.type}</h3>
                    ...
                    <div className="lg:col-span-3 bg-secondary rounded-lg p-6">
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-border">
                        <th className="text-left py-3 px-2 font-medium text-foreground">Security Control</th>
                        <th className="text-center py-3 px-2 font-medium text-foreground">Wootzapp Browser</th>
                        <th className="text-center py-3 px-2 font-medium text-foreground">Traditional VPN</th>
                        <th className="text-center py-3 px-2 font-medium text-foreground">VDI/DaaS</th>
                        <th className="text-left py-3 px-2 font-medium text-foreground">Deep Technical Implementation</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-border">
                      {[
                        ...
                        <tr key={index} className="hover:bg-secondary">
                          <td className="py-3 px-2 font-medium text-foreground">{row.control}</td>
                          <td className="py-3 px-2 text-center text-muted-foreground">{row.wootzapp}</td>
                          <td className="py-3 px-2 text-center text-muted-foreground">{row.vpn}</td>
                          <td className="py-3 px-2 text-center text-muted-foreground">{row.vdi}</td>
                          <td className="py-3 px-2 text-muted-foreground text-xs">{row.implementation}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4 md:gap-6 lg:gap-8 max-w-4xl mx-auto">
            {[
              ...
              <Card key={index} className="border-border bg-card shadow-sm text-center">
                <CardContent className="p-3 md:p-4 lg:p-6">
                  <cert.icon className="w-6 md:w-8 h-6 md:h-8 text-primary mx-auto mb-2 md:mb-3" />
                  <h4 className="font-medium text-foreground text-xs md:text-sm">{cert.name}</h4>
                </CardContent>
              </Card>
            ))}
          </div>
      ...
      <section className="py-12 md:py-16 lg:py-20">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-8 md:mb-12">
            ...
            <p className="text-muted-foreground max-w-2xl mx-auto px-4">Real-world performance metrics and scalability data</p>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 lg:gap-8 max-w-4xl mx-auto">
            {[
              ...
              <Card key={index} className="border-border bg-card shadow-sm text-center">
                <CardContent className="p-3 md:p-4 lg:p-6">
                  <div className="text-xl md:text-2xl mb-2 text-primary">{stat.icon}</div>
                  <div className="text-lg md:text-2xl font-bold text-foreground mb-1">{stat.metric}</div>
